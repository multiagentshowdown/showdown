    ...
    def render(self, env, delay=0.25):
        ...
        for h in range(0, env.height):
            for w in range(0, env.width):
                if terrain_dict[env.map[h, w]] == 'WALL':
                    pygame.draw.rect(
                        self.screen, WALL_C, [
                            self.scale * h, self.scale * w, self.scale, self.scale])
        ...
        for key_a, agent in env.agents.items():
            pygame.draw.circle(self.screen,
                               AGENT_C[key_a],
                               [int(agent.loc.c * self.scale + ceil(self.scale / 2)),
                                int(agent.loc.r * self.scale + ceil(self.scale / 2))],
                               ceil(self.scale * 0.3))
            ...
            for key_b, bullet in env.agents[key_a].bullets.items():
                pygame.draw.circle(self.screen,
                                   BULLET_C[key_a],
                                   [int(bullet.loc.c * self.scale + ceil(self.scale / 2)),
                                    int(bullet.loc.r * self.scale + ceil(self.scale / 2))],
                                   ceil(self.scale * 0.2))
